...
struct NotefieldImpl : public Notefield {
    ...
    void drawNotes() {
        ...
        for (auto& note : *gNotes) {
            ...
            // those.)
            if (isPreview && note.type != NOTE_MINE &&
                note.endrow < currentRow) {
                continue;
            }
            ...
            if (note.endrow > note.row) {
                ...
                // If we are doing chart preview, only show the part of the tail
                // past the targets, and don't show the arrow
                if (isPreview && currentRow >= note.row &&
                    currentRow <= note.endrow) {
            ...
            switch (note.type) {
                ...
                case NOTE_ROLL: {
                    int index =
                        (note.player * numCols + note.col) * NUM_ROW_TYPES +
                        rowtype;
    ...
    void drawGhostNote(const Note& n) override {
        ...
        // Render ghost step.
        int index =
            ((n.player * numCols) + n.col) * NUM_ROW_TYPES + ToRowType(n.row);
