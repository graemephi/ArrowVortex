...
#define MarkProgress(number, text)              \
    {                                           \
        if (data->terminate.stop_requested()) { \
            return;                             \
        }                                       \
...
struct GapData {
    GapData(int maxInterval, int downsample, int numOnsets,
            const Onset* onsets);
    ~GapData();
...
GapData::GapData(int bufferSize, int downsample, int numOnsets,
                 const Onset* onsets)
    : numOnsets(numOnsets),
      onsets(onsets),
...
static real GetConfidenceForBPM(const GapData& gapdata, IntervalTester& test,
                                real bpm) {
    int numOnsets = gapdata.numOnsets;
    const Onset* onsets = gapdata.onsets;
...
IntervalTester::~IntervalTester() { AlignedFree(fitness); }
...
static vec2i FillIntervalRange(IntervalTester& test, GapData& gapdata,
                               int begin, int end) {
...
static void RoundBPMValues(IntervalTester& test, GapData& gapdata,
                           TempoResults& tempo) {
    for (auto& t : tempo) {
...
static real GetBaseOffsetValue(const GapData& gapdata, int samplerate,
                               real bpm) {
    int numOnsets = gapdata.numOnsets;
    const Onset* onsets = gapdata.onsets;
...
static void CalculateOffset(SerializedTempo* data, Onset* onsets,
                            int numOnsets) {
    auto& tempo = data->result;
    int samplerate = data->samplerate;
...
static const char* sProgressText[]{
    "[1/6] Looking for onsets",  "[2/6] Scanning intervals",
    "[3/6] Refining intervals",  "[4/6] Selecting BPM values",
    "[5/6] Calculating offsets", "BPM detection results"};
...
TempoDetector* TempoDetector::New(double time, double len) {
    ...
    if (!music.isCompleted()) {
        HudInfo(
            "The music is still loading, wait a bit longer before using BPM "
            "detection.");
        return nullptr;
    }
