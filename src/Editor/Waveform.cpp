...
struct WaveformImpl : public Waveform {
    ...
    void sampleEdges(WaveEdge* edges, int w, int h, int channel, int blockId,
                     bool filtered) {
        auto& music = gMusic->getSamples();
        ...
        // A crash can occur if another thread is loading the audio. Just do
        // nothing if it is.
    ...
    void drawPeaks() override {
        ...
        for (; id * TEX_H < visibilityEndY; ++id) {
            ...
            Draw::fill({xl - pw, y, pw * 2, TEX_H}, waveCol, texL, uvs,
                       Texture::ALPHA);
            Draw::fill({xr - pw, y, pw * 2, TEX_H}, waveCol, texR, uvs,
                       Texture::ALPHA);
